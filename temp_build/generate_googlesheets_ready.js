...
async function createWorkbook() {
  ...
  const tasksRaw = [
    ...
    ['TASK-QA-001', 'Documentation & QA', 'Requirement traceability', 'Minha Palakkathodi', 'High', 'Day 1', 'None', 'RTM spreadsheet linking requirements to components code', 'No', 'No', 'Traceability index setup'],
  ...
  const demoCriticalTaskIds = new Set([
    'TASK-BE-004', 'TASK-BE-005', 'TASK-BE-012', 'TASK-BE-013', 'TASK-BE-014', 'TASK-BE-015',
    'TASK-FE-009', 'TASK-FE-010', 'TASK-FE-011', 'TASK-FE-012', 'TASK-FE-013', 'TASK-FE-014',
    'TASK-FE-015', 'TASK-FE-016', 'TASK-FE-017', 'TASK-DB-005', 'TASK-DB-006', 'TASK-DB-007',
    'TASK-DB-008', 'TASK-DB-009', 'TASK-DB-012', 'TASK-AI-003', 'TASK-AI-006', 'TASK-AI-007',
    'TASK-FR-004'
  ]);
  ...
  function autoFitColumns(sheet) {
    sheet.columns.forEach((column) => {
      ...
      if (sheet.name === 'Dashboard') {
        if (colNum === 11) width = 60; // Description
        else if (colNum === 12) width = 25; // Tech Stack
        else if (colNum === 13) width = 30; // Keywords
        else if (colNum === 14) width = 45; // Ref Docs
      } else if (timelines.includes(sheet.name)) {
        if (colNum === 8) width = 60; // Description
        ...
        if (colNum === 9) width = 60; // Description
  ...
  const kpis = [
    ...
    { label: 'Demo Critical Tasks Remaining', valFormula: '=COUNTIFS(F16:F111, "<>Completed", R16:R111, "Yes")', cellLabel: 'M2', cellVal: 'M3', mergeLabel: 'M2:N2', mergeVal: 'M3:N4', fmt: '0', result: demoCriticalCount },
  ...
  tasks.forEach((task, i) => {
    ...
    for (let col = 1; col <= 18; col++) {
      ...
      if (col === 1 || col === 5 || col === 6 || col === 8 || col === 9 || col === 15 || col === 16 || col === 18) {
  ...
  const guideRows = [
    ...
    ['5. How to update Last Updated field:', 'Go to your specific team task sheet (e.g. \'Frontend Tasks\') and type the date in dd-mm-yyyy format in the \'Last Updated\' column (Column O) whenever you change a task\'s status.'],
    ...
    ['8. How to prioritize Demo Critical tasks:', 'Filter the Master Task List on \'Dashboard\' by the \'Demo Critical?\' column (Column R) = \'Yes\'. These core features (Auth, Complaints, Attendance, Chatbot, etc.) must be completed first.']
  ];
  ...
  const t1Data = [
    ...
    ['Target Workspace Format', 'Google Sheets Native', 'Direct import via Google Drive conversion'],
    ['Formula Compatibility', '100% Compatible', 'All formulas use standard COUNTIF, COUNTIFS, and COUNTA'],
  ...
  const t2Data = [
    ...
    ['Conditional Formatting', 'Preserved', 'Import-compatible rules mapping status/priority colors'],
    ['Panes & Split Views', 'Preserved', 'Row freezing on Dashboard (14 rows) and timelines (2 rows)'],
  ...
  const t3Data = [
    ...
    ['1. Upload Workbook', 'Upload "Municipality_Project_Advanced_Kanban_GoogleSheets_Ready.xlsx" to Google Drive.', 'Check upload progress completes'],
    ['2. Convert Format', 'Right-click the file in Google Drive, select "Open with", and choose "Google Sheets".', 'Confirm file extension changes from .xlsx to Google Sheets format'],
    ['3. Verify Dropdowns', 'Open "Dashboard" and double-click any cell in column "Status" (Col F) or "Priority" (Col E).', 'Verify interactive list popup displays correctly'],
    ['4. Verify Formatting', 'Confirm Kanban Board status columns and priority colors are visible and styled.', 'Check background fills for Blocked, Completed, etc.'],
    ['5. Confirm View Freeze', 'Ensure the first 14 rows of the Dashboard and the first 2 rows of task sheets remain frozen.', 'Scroll vertically and verify frozen header row behaves correctly']
  ];
  ...
  const destPath = path.join(__dirname, '..', 'Municipality_Project_Advanced_Kanban_GoogleSheets_Ready.xlsx');
