...
async function verify() {
  ...
  for (let i = 0; i < expectedDashHeaders.length; i++) {
    if (dashHeaders[i] !== expectedDashHeaders[i]) {
      throw new Error(
        `Header mismatch at index ${i}: Expected "${expectedDashHeaders[i]}", but found "${dashHeaders[i]}"`
      );
    }
  }
  ...
  const kpisToCheck = [
    {
      ...
      expectedFormula: '=COUNTIF(F16:F111, "Completed")/COUNTA(A16:A111)',
    },
    { cell: 'C3', label: 'Blocked Tasks', expectedFormula: '=COUNTIF(F16:F111, "Blocked")' },
    {
      cell: 'E3',
      label: 'Critical Issues Open',
      expectedFormula: '=COUNTIFS(E16:E111, "Critical", F16:F111, "<>Completed")',
    },
    {
      cell: 'M3',
      label: 'Demo Critical Tasks Remaining',
      expectedFormula: '=COUNTIFS(F16:F111, "<>Completed", R16:R111, "Yes")',
    },
  ];
  ...
  kpisToCheck.forEach((k) => {
    ...
    if (cell.value.formula !== k.expectedFormula) {
      throw new Error(
        `Formula mismatch at cell ${k.cell}: Expected "${k.expectedFormula}", but found "${cell.value.formula}"`
      );
    }
  });
  ...
  for (let i = 0; i < expectedTimelineHeaders.length; i++) {
    if (sprint0Headers[i] !== expectedTimelineHeaders[i]) {
      throw new Error(
        `Sprint 0 header mismatch at index ${i}: Expected "${expectedTimelineHeaders[i]}", but found "${sprint0Headers[i]}"`
      );
    }
  }
  ...
  for (let i = 0; i < expectedModuleHeaders.length; i++) {
    if (beHeaders[i] !== expectedModuleHeaders[i]) {
      throw new Error(
        `Backend Tasks header mismatch at index ${i}: Expected "${expectedModuleHeaders[i]}", but found "${beHeaders[i]}"`
      );
    }
  }
  ...
  if (sampleRowRemarksValue.formula !== '=Dashboard!Q41') {
    throw new Error(
      `Backend Tasks Row 3 Remarks formula should be "=Dashboard!Q41", but is "${sampleRowRemarksValue.formula}"`
    );
  }
  ...
  if (sampleRowTestValue.formula !== '=Dashboard!O41') {
    throw new Error(
      `Backend Tasks Row 3 Testing Status formula should be "=Dashboard!O41", but is "${sampleRowTestValue.formula}"`
    );
  }
  ...
  if (repTitleVal !== 'Google Sheets Migration & Compatibility Report') {
    throw new Error(
      `Expected title "Google Sheets Migration & Compatibility Report", but found "${repTitleVal}"`
    );
  }
